...
export const EmployeeNewPosition = () => {
  ...
  const {
    ...
    url: `home/${
      id == "aj" ? "role/" : ""
    }find-all-new-employee?page=${currentPage}&limit=${pageSize}${
      date ? `&month=${date}` : ""
  ...
  return (
    <>
      <Breadcrumb
        link={`Ishga yangi kirganlar ${id == "aj" ? "(AJ)" : ""}`}
        link1={"Bosh sahifa"}
        link2={`Ishga yangi kirganlar ${id == "aj" ? "(AJ)" : ""}`}
      />
      <div className="p-6 rounded-lg bg-[var(--bgWhite-color)]">
        <div className="grid grid-cols-4 gap-2 overflow-x-hidden ease-in duration-500 max-lg:w-full max-sm:grid-cols-2">
          ...
          <DatePicker
            ...
            type="date"
            size="middle"
            className=" w-full border-[var(--border-color)] hover:border-[var(--text-color)] focus:border-[var(--text-color)]"
          />
        </div>
