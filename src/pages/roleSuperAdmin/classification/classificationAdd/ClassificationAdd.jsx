...
export const ClassificationAdd = () => {
  ...
  const {
    ...
    url: `classification/find/all?page=${currentPage}&limit=10&lang=uz`,
  });
  //   gruhni olish
  const {
    data: { data: dataGroup },
  ...
  const deleteConfirm = (id) => {
    mutate({
      url: `classification/delete-classification/${id}`,
      method: "DELETE",
      onSuccess: () => {
        refetch();
  ...
  return (
    <>
      <Breadcrumb
        link={"Klassfikator qo'shish"}
        link1={"Klassfikator"}
        link2={"Klassfikator qo'shish"}
      />
              ...
              {data?.items?.map((e, i) => {
                return (
                  ...
                  >
                    ...
                    <td className="tracking-normal leading-[130%] text-[var(--textBlack-color)] text-sm table-cell align-middle border border-solid border-[var(--borderWhite-color)] p-2 text-start border-l-0 border-r-0">
                      {e?.group?.names?.map((e, i) => (
                        <p className="mt-2 text-[var(--text-color)]" key={i}>
                          {e}
                        </p>
                      ))}
                    </td>
