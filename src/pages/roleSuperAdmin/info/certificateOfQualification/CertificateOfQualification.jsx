...
export const CertificateOfQualification = () => {
  ...
  const {
    ...
    url: `qualification-certificate-type/find/all`,
  });
  ...
  const onSubmit = (form) => {
    if (editId != -1) {
      mutate({
        url: "qualification-certificate-type/edit",
        method: "PUT",
        data: {
          id: editId,
          name: form.name,
      ...
      mutate({
        url: "qualification-certificate-type/add",
        method: "POST",
        data: {
          name: form.name,
        },
  ...
  const deleteConfirm = (id) => {
    mutate({
      url: `qualification-certificate-type/delete/${id}`,
      method: "DELETE",
      onSuccess: () => {
        refetch();
  ...
  return (
    <>
      <Breadcrumb
        link={"Guvohnoma (malakaoshirish) turi"}
        link1={"Guvohnoma (malakaoshirish) turi"}
        link2={"Guvohnoma (malakaoshirish) turi"}
      />
      <div className="p-6 rounded-lg bg-[var(--bgWhite-color)]">
        <div className="w-full flex justify-end  mb-6">
          ...
          >
            Guvohnoma (malakaoshirish) qo'shish
          </button>
        </div>
        ...
        <Modal
          title="Guvohnoma (malakaoshirish) qo'shish"
          open={openModal}
          onOk={showModal}
            ...
            <div className="w-full">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    {...field}
                    placeholder="Guvohnoma (malakaoshirish) nomi"
                    size="large"
